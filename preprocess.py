...
def run(working_set):
    for gid, game in working_set.items():
        ...
        print(f'Analyzing {gid} ({len(moves)} moves)')
        ...
        for board, played_move in reversed(moves):
            ...
            move = Move.create(game=game_obj, color=board.turn, number=board.fullmove_number, \
                               pv1_eval=evals.get(1), pv2_eval=evals.get(2), pv3_eval=evals.get(3), \
                               pv4_eval=evals.get(4), pv5_eval=evals.get(5), \
                               played_index=played_index, played_eval=played_eval)
